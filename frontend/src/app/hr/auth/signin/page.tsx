      ...
      <div className="w-full max-w-md flex flex-col gap-4">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-200 p-8">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            {GOOGLE_CLIENT_ID && (
              <>
                <div className="relative my-4"><div className="absolute inset-0 flex items-center"><span className="w-full border-t border-slate-300" /></div><div className="relative flex justify-center text-sm"><span className="px-2 bg-white text-slate-500 font-figtree">or</span></div></div>
                <button
                  type="button"
                  onClick={() => {
          ...
          <div className="mt-8 pt-6 border-t border-gray-100 text-center">
            <p className="text-sm text-gray-500 font-figtree">
              Are you a job seeker? <Link href="/candidate/auth/signin" className="text-[#2D2DDD] font-semibold hover:underline">Candidate Login</Link>
            </p>
          </div>
        </motion.div>
      </div>
