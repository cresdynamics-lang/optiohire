...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/hr/candidates/${candidateId}`, {
      headers: {
    ...
    console.error('Proxy GET hr-candidate by id error:', err)
...
export async function PATCH(
  ...
  try {
    ...
    console.error('Proxy PATCH hr-candidate error:', err)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
