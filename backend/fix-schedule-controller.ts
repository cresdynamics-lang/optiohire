...
// Add missing import
if (!content.includes('CompanyUserRepository')) {
  content = content.replace(
    /import { JobPostingRepository } from '\.\.\/repositories\/jobPostingRepository\.js'/,
    "import { JobPostingRepository } from '../repositories/jobPostingRepository.js'\nimport { CompanyUserRepository } from '../repositories/userRepository.js'"
  )
}

// Replace the buggy bulkScheduleInterview
const newBulk = `
export async function bulkScheduleInterview(req: Request, res: Response) {
  try {
    ...
    const userRepo = new CompanyUserRepository()
...
console.log('Fixed typescript errors in schedule controller')
