 ...
 */
export async function uploadPublicResume(req: Request, res: Response) {
  try {
    if (!req.file) {
      return res.status(400).json({ error: 'No file uploaded' })
    }
    ...
    const safeExt = path.extname(file.originalname).toLowerCase() || '.pdf'
    const uniqueId = crypto.randomBytes(16).toString('hex')
    // Store public uploads in a separate directory
    const filename = `public-resumes/${uniqueId}${safeExt}`

    const fileUrl = await saveFile(filename, file.buffer)

    logger.info('Public resume uploaded successfully', {
      filename,
      size: file.size,
      mimetype: file.mimetype,
      originalname: file.originalname,
    ...
    logger.error('Error uploading public resume:', error)
    return res.status(500).json({
