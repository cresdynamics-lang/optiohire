...
export async function authenticate(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    // SECURITY: Admin bypass mechanism - only allow in development or with secure token
    ...
    // 2. Valid admin token is provided (production-safe)
    const allowAdminBypass = process.env.NODE_ENV === 'development' || 
      (ADMIN_SECRET_TOKEN && adminToken === ADMIN_SECRET_TOKEN)
...
/**
 * Optional auth: sets req.userId, req.userEmail, req.userRole when Bearer token is valid.
 * Does not require company; does not return 401 when no token (so company-setup can link user_id).
 */
