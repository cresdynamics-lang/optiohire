 ...
 * Run from backend directory: cd backend && node ../scripts/create-sales-job-db.js
 */
...
const pool = new Pool({
  connectionString: process.env.DATABASE_URL || 'postgresql://optiohire_user:optiohire_pass@localhost:5432/optiohire'
})
  ...
  try {
    ...
    if (companyResult.rows.length > 0) {
      ...
      const insertCompany = await client.query(
        `INSERT INTO companies (company_name, company_email, hr_email, company_domain, created_at, updated_at)
         VALUES ($1, $2, $3, $4, now(), now())
         RETURNING company_id`,
        ['Cres Dynamics', 'hr@cresdynamics.com', 'hr@cresdynamics.com', 'cresdynamics.com']
      )
    ...
    const jobResult = await client.query(
      ...
      [
        ...
        'We are looking for an experienced Sales professional to join our team. The ideal candidate will have a proven track record in sales, excellent communication skills, and the ability to build strong client relationships. Responsibilities include identifying new business opportunities, managing client accounts, and achieving sales targets.',
        'Sales, Customer Relations, Communication, Negotiation, CRM',
